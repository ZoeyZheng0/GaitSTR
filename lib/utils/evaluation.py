...
def de_n(acc):
    dividend = acc.shape[1]
    result = np.sum(acc, 1) / dividend    
    return result
...
def identification(data, dataset, metric='euc'):
    ...
    if 'OUMVLP' not in dataset:
        ...
        #         np.mean(acc[1, :, :, i]),
        #         np.mean(acc[2, :, :, i])))
        # msg_mgr.log_info('NM: {}'.format(de_n(acc[0, :, :, 0])))
        # msg_mgr.log_info('BG: {}'.format(de_n(acc[1, :, :, 0])))
        # msg_mgr.log_info('CL: {}'.format(de_n(acc[2, :, :, 0])))
        for i in range(1):
            msg_mgr.log_info(
