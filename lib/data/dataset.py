...
class DataSet(tordata.Dataset):
    ...
    def __loader__(self, paths):
        ...
        for pth in paths:
            if pth.endswith('.pkl'):
                ...
                _o = np.load(pth[:-4] + '_orientation.npy')
            else:
                raise ValueError('- Loader - just support .pkl !!!')
            gait_list.append(_g)  # pose, gait
            pose_list.append(_p)
            orientation_list.append(_o)

        # return gait_list, pose_list
        return gait_list, pose_list, orientation_list
    ...
    def __getitem__(self, idx):
        ...
        # return data_lst[1], seq_info, data_lst[0]
        return data_lst[1], data_lst[2], seq_info, data_lst[0]
    ...
    def __dataset_parser(self, data_config, training):
        ...
        def get_seqs_info_list(label_set, dtrain):
            seqs_info_list = []
        ...
        self.seqs_info = get_seqs_info_list(
            train_set, True) if training else get_seqs_info_list(test_set, False)
