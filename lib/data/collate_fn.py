...
class CollateFn(object):
    ...
    def __call__(self, batch):
        ...
        pose_batch, orientation_batch = [], []
        for bt in batch:
            pose_batch.append(bt[0]) # pose
            # labs_batch.append(self.label_set.index(bt[1][0]))
            # typs_batch.append(bt[1][1])
            # vies_batch.append(bt[1][2])
            # seqs_batch.append(bt[2])
            orientation_batch.append(bt[1]) # orientation
            labs_batch.append(self.label_set.index(bt[2][0]))
            typs_batch.append(bt[2][1])
            vies_batch.append(bt[2][2])
            seqs_batch.append(bt[3])
        global count
        count = 0
        ...
        batch = [pose_batch, orientation_batch, fras_batch, labs_batch, typs_batch, vies_batch, None]
        if self.sampler == "fixed":
